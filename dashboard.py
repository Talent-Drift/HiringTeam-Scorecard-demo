...
# Removed import of scoring_engine - will handle this with error checking
...
def load_data():
    ...
    try:
        # Try to import scoring_engine
        try:
            from scoring_engine import ScorecardEngine
        except ImportError:
            st.error("❌ Missing dependency: `scoring_engine.py` not found")
            st.info("Please ensure `scoring_engine.py` is in the same directory as this dashboard")
            return None
        ...
        st.info("Please generate sample data first by running: `python generate_sample_data.py`")
        return None
...
def main():
    ...
    if data is None:
        st.warning("⚠️ Unable to load data. Please check the requirements below:")
        st.markdown("""
        **Required files:**
        1. `scoring_engine.py` - The scoring engine module
        2. `sample_ats_export.csv` - Sample data file
    ...
    page = st.sidebar.radio(
        "Select View",
        ["Open Roles Dashboard", "Organization Overview", "Recruiter Cards", "Hiring Manager Cards"]
    )
